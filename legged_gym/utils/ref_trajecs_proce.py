...
PATH_CONSTANT_SPEED = (
    LEGGED_GYM_ROOT_DIR
    + "/resources/trajectory/drloco_mat/Trajecs_Constant_Speed_400Hz.mat"
)
PATH_SPEED_RAMP = (
    LEGGED_GYM_ROOT_DIR
    + "/resources/trajectory/drloco_mat/Trajecs_Ramp_Slow_400Hz_EulerTrunkAdded.mat"
)
...
FOOT_POSX_L, FOOT_POSY_L, FOOT_POSZ_L, FOOT_POSX_R, FOOT_POSY_R, FOOT_POSZ_R = range(
    29, 35
)
...
class DrlocoReferenceTrajectories:
    ...
    def next(self):
        ...
        for i in range(len(self.step)):
            ...
            if self.step[i] >= self.env_traj_len[i, 1]:
                ...
                self.env_traj_len[i, 1] = self.cell_max_traj_len[
                    self.env_traj_len[i, 0]
                ]
    ...
    def get_ref_observation(self):
        ...
        for i in range(self.num_env):
            ...
            self.state_full[i, :] = self.data[self.env_traj_len[i, 0]][
                self._state_indices, self.step[i]
            ]

        # 3d_hip: add hip traversal value
        angle_add = 0.05 * np.ones([self.num_env, 1])
        vel_add = np.zeros([self.num_env, 1])
        self.state_full_3d = np.concatenate(
            [
                self.state_full[:, :12],
                -angle_add,
                self.state_full[:, 12:16],
                angle_add,
                self.state_full[:, 16:20],
                vel_add,
                self.state_full[:, 20:24],
                vel_add,
                self.state_full[:, 24:26],
            ],
            axis=1,
        )
